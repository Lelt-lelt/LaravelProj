...
<div class="card shadow">
	...
	<div class="body">
		...
		<form action="{{route('showtimes.update',$showtime->id)}}" method="post" enctype="multipart/form-data">
			@csrf
			@method('PUT')
			<div class="form-group row">
				...
				<div class="col-sm-10">
					<input type="text" class="form-control @error('name') is-invalid @enderror" id="showtime" placeholder="Enter Show time" name="showtime" value="{{$showtime->showtime}}" >
					@error('name')
			...
			<div class="form-group row">
				...
				<div class="col-sm-10">
					<input type="number" class="form-control @error('name') is-invalid @enderror" id="noofseat" placeholder="Enter No of seat" name="noofseat" value="{{$showtime->no_of_seat}}" >
					@error('name')
					<span class="invalid-feedback" role="alert">
						<strong>{{ $message }}</strong>
					</span>
				...
				<label for="rundate" class="col-sm-2 col-form-label">Run Date</label>
					...
					<select class="form-control custom-select" name="rundate">
						<option selected>Choose Starting Date</option>
							...
							<option value="{{$row->id}}"
								@if($row->id==$showtime->run_date_id)
								selected
							@endif 
								>	{{$row->starting_date}}
							</option>
